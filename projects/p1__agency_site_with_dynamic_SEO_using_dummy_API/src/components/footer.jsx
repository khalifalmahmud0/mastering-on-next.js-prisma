...
const fetchData = async () => {
    try {
        let fetching = await fetch(process.env.API_URL+'SocialLink');
                        ...
                        <span className="inline-flex sm:ml-auto sm:mt-0 mt-4 justify-center sm:justify-start">
                            {data?.map((item,index) => {
                                ...
                                if (item?.name === 'facebook'){
                                     icon =  <svg fill="currentColor" strokeLinecap={"round"} strokeLinejoin={"round"}
                                        strokeWidth={"2"}
                                        className="w-5 h-5"
                                        viewBox="0 0 24 24">
                                     ...
                                     icon = <svg fill="none" stroke="currentColor" strokeLinecap={"round"} strokeLinejoin={"round"}
                                        strokeWidth={"2"}
                                ...
                                return (
                                    <Link key={index} href={item?.link ? item?.link : '#'} className="mx-3 text-gray-500">
                                        {icon}
                                    </Link>
                                )
                            })}
